...
void ShowStuInfo() // 打印学生信息
{
  if (stuCnt == 0) {
    ...
    for (int i = 1; i <= stuCnt; i++) {
      printf("*  %s  *\t%s\t*\t%s\t*\t%s\t*\t%s\t*\n", stu[i].id, stu[i].name,
             stu[i].sex, stu[i].dorm, stu[i].tel);
      printf("*****************************************************************"
             "************************\n");
    }
  }
...
void AddStuInfo() // 新增学生信息
{
  ...
  for (int j = 1; j < M; j++) {
    ...
    getchar();
    grade[stuCnt].course[j].totalGrade =
        0.4 * grade[stuCnt].course[j].triGrade +
        0.6 * grade[stuCnt].course[j].examGrade; // 综合总成绩
  }
  for (int j = 1; j < M; j++) // 计算学生总成绩和每门课程的实得学分
  {
    grade[stuCnt].course[j].finalCredit =
        grade[stuCnt].course[j].totalGrade < 60
            ? 0
            : grade[stuCnt].course[j].credit; // 只有及格得到学分
...
void DeleteStuInfo() // 删除学生信息
{
  ...
  for (int i = 1; i <= stuCnt; i++) {
    if (num == 1 && strcmp(str, stu[i].id) == 0) // 根据学号删除
    {
      ...
      (i == stuCnt ? strcpy(grade[stuCnt].stuId, "NULL")
                   : strcpy(grade[stuCnt].stuId, grade[stuCnt + 1].stuId));
      stuCnt--;
      printf("删除成功，请记得保存。\n");
...
void ReadStuInfo() // 读取学生信息
{
  ...
  while (fscanf(pFile1, "%s", str) != EOF) // 读到文件尾为止
  {
    ...
    case 0:
      ...
      break;
    case 1:
      strcpy(stu[stuCnt].name, str);
      break;
    case 2:
      strcpy(stu[stuCnt].sex, str);
      break;
    case 3:
      strcpy(stu[stuCnt].dorm, str);
      break;
    case 4:
      strcpy(stu[stuCnt].tel, str);
      break;
    default:
      stuCnt++;
      strcpy(stu[stuCnt].id, str);
    ...
    for (int i = 1; i <= stuCnt; i++) {
      ...
      for (int j = 1; j < M; j++) {
        printf("*\t%s\t\t*\t%s\t\t*\t%d\t*\t%d\t\t*\t%d\t\t*\t%f\t*\t%d\t\t*\n",
               grade[i].course[j].id, grade[i].course[j].name,
               grade[i].course[j].credit, grade[i].course[j].triGrade,
               grade[i].course[j].examGrade, grade[i].course[j].totalGrade,
